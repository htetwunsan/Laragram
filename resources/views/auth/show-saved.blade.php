<x-base-layout>
    <section id="section_main" class="h-full flex flex-col items-stretch overflow-y-auto">
        ...
        <x-app-top-navigation>
            <x-slot name="left">
                <button id="btn_open_menu" type="button">
                    <svg aria-label="Options" class="text-triple38 w-6 h-6 fill-current"
            ...
            <x-slot name="right">
                <button class="flex items-center justify-center p-2"
                        type="button"
                        onclick="window.location.href = '{{ route('user.explore') }}';">
                    <svg aria-label="Discover People" class="text-triple38 w-6 h-6 fill-current" role="img"
                         viewBox="0 0 48 48">
                        <path
        ...
        <main class="bg-white flex-grow flex flex-col items-stretch">
            <div class="flex-grow flex flex-col items-stretch">
                ...
                @component('components.auth.profile', ['user' => $user])@endcomponent

                @component('components.auth.tab-header', ['user' => $user, 'activeTab' => $activeTab])@endcomponent

                @component('components.user.saved', ['posts' => $posts])@endcomponent
            </div>
        </main>
    </section>

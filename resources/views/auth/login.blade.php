<x-base-layout>
        ...
        <main class="flex flex-col flex-grow">
            <div class="flex flex-col min-h-screen">
                ...
                <article class="flex flex-grow items-stretch justify-center pb-8">
                    <div class="flex flex-grow flex-col pb-16 items-center">
                        <div class="flex flex-col items-center py-2.5">
                            <h1 class="font-cookie font-semibold text-5xl text-triple38 tracking-wider mt-6 mb-3">{{
                                config('app.name', 'Laragram') }}</h1>
                        </div>
                        <div class="flex flex-col items-stretch" style="width: 350px">
                            <form class="flex flex-col items-stretch mt-6" method="POST" action="{{ route('login') }}">
                                @csrf
                                <div class="flex flex-col items-stretch my-2 mx-10">
                                    <a class="bg-fb_blue flex flex-row gap-x-1 items-center justify-center h-8 appearance-none py-1 px-2 border border-transparent border-solid rounded leading-5"
                                        tabindex="0" href="{{ route('facebook.login') }}">
                                        <x-fb-icon />
                                        <span
                                            class="block text-white text-sm font-bold leading-4 tracking-normal">Continue
                                            with Facebook</span>
                                    </a>
                                </div>
                                ...
                                <div class="flex flex-col items-stretch mx-10 mb-2">
                                    <div
                                        ...
                                        <label class="h-9 relative">
                                            ...
                                            <span
                                                class="floating-label h-9 leading-9 absolute left-2 text-xs text-triple142 select-none pointer-events-none">Username
                                                or email</span>
                                        </label>
                                    </div>
                                </div>
                                ...
                                <div class="flex flex-col items-stretch mx-10 my-2.5">
                                    ...
                                    <span class="text-sm text-error text-center font-normal leading-18px">{{ $error
                                        }}</span>
                                    @endforeach
                                </div>
                                @endif
                ...
                <div class="flex flex-col items-center justify-center mb-6">
                    ...
                    <span class="block font-sans font-semibold text-sm text-triple38 tracking-widest">FACEBOOK</span>
                </div>
            </div>
        </main>
        <footer class="flex-none flex flex-col items-stretch bg-triple250 px-4 order-last">
            <div class="flex flex-col items-stretch mb-3">
                <div class="flex flex-col items-stretch mt-6 justify-start">
                    <div class="flex flex-wrap items-stretch justify-center">
                        <span class="block text-xs text-triple142 leading-4 mx-2 mb-2">Meta</span>
                        ...
                        <span class="block text-xs text-triple142 leading-4 mx-2 mb-2">Top Accounts</span>
                        ...
                        <span class="block text-xs text-triple142 leading-4 mx-2 mb-2">Instagram Lite</span>
                    </div>
                    ...
                    <div class="flex flex-wrap items-stretch justify-center">
                        ...
                        <span class="block text-xs text-triple142 leading-4 mx-2 mb-2">Food & Drink</span>
                        <span class="block text-xs text-triple142 leading-4 mx-2 mb-2">Home & Garden</span>
                        <span class="block text-xs text-triple142 leading-4 mx-2 mb-2">Music</span>
                        <span class="block text-xs text-triple142 leading-4 mx-2 mb-2">Visual Arts</span>
                    </div>
                </div>
            </div>
        </footer>
    ...
    <script>
        $(document).ready(function () {
                ...
                function validate() {
                    const emailValid = $inputEmail.val().length > 0;
                    const passwordValid = $inputPassword.val().length >= 8;

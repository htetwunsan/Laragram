    ...
    <div class="flex-shrink-0 flex flex-col items-stretch mr-7">
        <div style="width: 77px; height: 77px;">
            <a href="{{ route('user.story.index', ['user' => $user->username]) }}">
                ...
                <span class="flex items-center justify-center rounded-full p-0.5 mx-auto mt-1 mb-1.5">
                    <span class="block bg-white border border-triple219 rounded-full p-0.5" role="link" tabindex="-1">
                        <x-profile-image class="w-full h-full rounded-full"
                            title="{{ $user->username }}'s profile photo" alt="{{ $user->username }}'s profile photo">
                            {{ $user->profile_image }}
                        </x-profile-image>
                    </span>
                </span>
                ...
                <span
                    ...
                    <span class="block bg-white rounded-full p-0.5" role="link" tabindex="-1">
                        <x-profile-image class="w-full h-full rounded-full"
                            title="{{ $user->username }}'s profile photo" alt="{{ $user->username }}'s profile photo">
                            {{ $user->profile_image }}
                        </x-profile-image>
    ...
    <section class="flex flex-col items-stretch overflow-hidden">
        <div class="flex flex-col items-stretch mb-3">
            <div class="flex items-center justify-center">
                ...
                <div class="flex items-center justify-center w-12" style="margin-left: 5px">
                    <button id="btn_user_more_options" class="p-2" type="button">
                        <svg aria-label="Options" class="_8-yf5 " color="#262626" fill="#262626" height="32" role="img"

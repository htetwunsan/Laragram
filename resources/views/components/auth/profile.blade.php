<header class="flex flex-row items-stretch mx-4 mt-4 mb-6">
    <div class="flex-shrink-0 flex flex-col items-stretch mr-7">
        <form id="form_update_profile_image" action="{{ route('auth.update.profile-image') }}" method="POST"
            ...
            @csrf
            @method('PATCH')
            <input id="input_profile_image" class="hidden" type="file" accept="image/*" name="profile_image" />
        </form>
        <button id="btn_add_profile_image" style="width: 77px; height: 77px" type="button" title="Add a profile photo">
            <x-profile-image class="w-full h-full rounded-full" title="Add a profile photo" alt="Add a profile photo">
                {{ $user->profile_image }}
            </x-profile-image>
        </button>
    </div>
    <section class="flex-grow flex flex-col items-stretch overflow-hidden">
        <div class="flex flex-col items-stretch mb-3">
            <h2 class="text-triple38 font-light leading-8 -mt-1.5 -mb-1.5 overflow-hidden overflow-ellipsis whitespace-nowrap"
        ...
        <div class="flex flex-col items-stretch" style="max-width: 250px; tab-index: 0">
            <a class="text-sm text-triple38 text-center font-semibold leading-18px border border-solid border-triple219 rounded"
                href="{{ route('auth.edit') }}" style="padding:5px 9px;">Edit
                Profile</a>
        </div>
    </section>
</header>
...
<x-app-dialog id="div_confirm_profile_image_dialog">
    ...
    <button class="text-sm text-fb_blue text-center font-bold h-12 py-1 px-2 border-t border-triple219" type="button"
        tabindex="0"
        onclick="window.toggleDialog($('#div_confirm_profile_image_dialog')); $('#input_profile_image').trigger('click');">
        Upload photo
    </button>

    <form class="flex flex-col items-stretch" method="POST" action="{{ route('auth.destroy.profile-image') }}">@csrf
        @method('DELETE')
        <button class="text-sm text-error text-center font-bold h-12 py-1 px-2 border-t border-triple219" type="submit"
            tabindex="0">
    ...
    <button class="btn_cancel text-sm text-triple38 text-center font-bold h-12 py-1 px-2 border-t border-triple219"
        type="button" tabindex="0" onclick="window.toggleDialog($('#div_confirm_profile_image_dialog'));">
        Cancel
    </button>
</x-app-dialog>
